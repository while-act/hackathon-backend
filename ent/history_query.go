...
func (hq *HistoryQuery) QueryIndustry() *IndustryQuery {
	query := (&IndustryClient{config: hq.config}).Query()
...
func (hq *HistoryQuery) QueryUsers() *UserQuery {
...
func (hq *HistoryQuery) WithIndustry(opts ...func(*IndustryQuery)) *HistoryQuery {
	query := (&IndustryClient{config: hq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	hq.withIndustry = query
	return hq
}
...
func (hq *HistoryQuery) WithUsers(opts ...func(*UserQuery)) *HistoryQuery {
	query := (&UserClient{config: hq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (hq *HistoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*History, error) {
	...
	if query := hq.withIndustry; query != nil {
		if err := hq.loadIndustry(ctx, query, nodes, nil,
			func(n *History, e *Industry) { n.Edges.Industry = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := hq.withUsers; query != nil {
		if err := hq.loadUsers(ctx, query, nodes, nil,
			func(n *History, e *User) { n.Edges.Users = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (hq *HistoryQuery) loadIndustry(ctx context.Context, query *IndustryQuery, nodes []*History, init func(*History), assign func(*History, *Industry)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*History)
	for i := range nodes {
		fk := nodes[i].IndustryBranch
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "industry_branch" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (hq *HistoryQuery) loadUsers(ctx context.Context, query *UserQuery, nodes []*History, init func(*History), assign func(*History, *User)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*History)
	for i := range nodes {
		fk := nodes[i].UserID
